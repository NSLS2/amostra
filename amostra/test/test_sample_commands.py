...
def test_connection_switch(amostra_server, amostra_client):
    ...
    amostra_client.create_sample(name='asterix')
...
def test_find_sample(amostra_server, amostra_client):
    ...
    s_ret = amostra_client.find_sample(uid=m_sample['uid'])[0]
    assert Document == type(s_ret)
    assert s_ret['uid'] == m_sample['uid']
...
def test_find_sample_as_doc(amostra_server, amostra_client):
    ...
    amostra_client.create_sample(**m_sample)
    s_ret = amostra_client.find_sample(uid=m_sample['uid'], as_document=True)[0]
